"""
macro_btc_agent.py — วิเคราะห์ macro trend จากกราฟ BTC 4H และ 1D
...
class MacroBTCAgent(BaseAgent):
    """
    วิเคราะห์ BTC macro trend ด้วย 4 rules:
    1. Market Structure (HH/HL vs LH/LL) บน 1D
    2. EMA 200 บน 4H
    3. Weekly trend (7 วัน)
    4. Monthly trend (30 วัน)
    """

    BTC_SYMBOL = "BTC/USDT:USDT"

    def __init__(self, data_fetcher, db):
        super().__init__("macro_btc", data_fetcher, db)
    ...
    async def analyze(self) -> AgentSignal:
        ...
        try:
            ...
            if len(df_1d) >= 8:
                ...
                if price_7d_ago > 0:
                    ...
                    if weekly_change >= 0.05:
                        score += 2
                        reasons.append(f"BTC Weekly +{weekly_change:.1%} bullish +2")
                    elif weekly_change <= -0.05:
                        score -= 2
                        reasons.append(f"BTC Weekly {weekly_change:.1%} bearish -2")
                    else:
                        reasons.append(f"BTC Weekly {weekly_change:+.1%} neutral")

            # Rule 4 — Monthly trend
            if len(df_1d) >= 32:
                ...
                if price_30d_ago > 0:
                    ...
                    if monthly_change >= 0.15:
                        score += 2
                        reasons.append(f"BTC Monthly +{monthly_change:.1%} strong bull +2")
                    elif monthly_change <= -0.15:
                        score -= 2
                        reasons.append(f"BTC Monthly {monthly_change:.1%} strong bear -2")
                    else:
                        reasons.append(f"BTC Monthly {monthly_change:+.1%} neutral")
            ...
            logger.error(f"MacroBTCAgent analyze error: {e}")
